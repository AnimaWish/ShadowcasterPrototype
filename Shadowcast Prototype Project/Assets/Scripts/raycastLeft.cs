...
public class raycastLeft : MonoBehaviour
{
    ...
    void FixedUpdate()
    {
        ...
        if (Physics.Raycast(transform.position, left, out hit) && hit.collider.name == "Box")
        {
            Physics.Raycast(hit.transform.position, left, out hit2, 30, LayerMask.GetMask("wall"));
